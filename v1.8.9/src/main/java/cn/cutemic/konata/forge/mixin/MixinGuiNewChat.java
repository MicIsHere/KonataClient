...
import cn.cutemic.konata.forge.api.IChatLine;
import cn.cutemic.konata.features.impl.interfaces.BetterChat;
...
public abstract class MixinGuiNewChat {
    ...
    public void drawChat(int updateCounter) {
        if (mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            if (!BetterChat.using) {
                ...
                BetterChat module = (BetterChat) Konata.moduleManager.getModule(BetterChat.class);
                            ...
                            if (n < 200 || bl) {
                                ...
                                if (alpha > 3) {
                                    ...
                                    if (module.betterFont.getValue()) {
                                        Konata.fontManager.s16.drawStringWithShadow(string, 0.0F, (float) (q - 8) + (6 - (alpha / 255f) * 6), Render2DUtils.reAlpha(new Color(16777215), alpha).getRGB());
                                    } else {
        ...
        if (!this.getChatOpen()) {
            ...
            if (j >= 0 && k >= 0) {
                ...
                if (BetterChat.using && module.betterFont.getValue()) {
                    fontHeight = Konata.fontManager.s16.getHeight();
                }
                    ...
                    if (m >= 0 && m < this.drawnChatLines.size()) {
                        ...
                        while (var12.hasNext()) {
                            ...
                            if (iTextComponent instanceof ChatComponentText) {
                                if (BetterChat.using && module.betterFont.getValue()) {
                                    n += Konata.fontManager.s16.getStringWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText) iTextComponent).getChatComponentText_TextValue(), false));
                                } else {
    ...
    public List<IChatComponent> spilt(IChatComponent chatComponent, int i, FontRenderer chatcomponenttext, boolean l, boolean chatcomponenttext2){
        ...
        if (BetterChat.using && module.betterFont.getValue()) {
            return GuiUtilRenderComponents.splitText(chatComponent, i, Konata.fontManager.s16, false, false);
        } else {
