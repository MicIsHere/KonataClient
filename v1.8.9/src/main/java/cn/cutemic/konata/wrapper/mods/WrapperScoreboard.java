...
import cn.cutemic.konata.ui.custom.impl.ScoreboardComponent;
...
public class WrapperScoreboard {
    public static float[] render(ScoreboardComponent scoreboardComponent, InterfaceModule mod, float x, float y) {
        cn.cutemic.konata.wrapper.scoreboard.WrapperScoreboard scoreboard = new cn.cutemic.konata.wrapper.scoreboard.WrapperScoreboard(ProviderManager.worldClientProvider.getWorld().getScoreboard());
        ScoreObjective scoreobjective = null;
        ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(ProviderManager.mcProvider.getPlayer().getName());

        UFontRenderer s16 = Konata.fontManager.s16;
