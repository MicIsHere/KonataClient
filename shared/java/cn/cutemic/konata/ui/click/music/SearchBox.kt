...
import cn.cutemic.konata.utils.math.MathUtils.clamp
...
class SearchBox : Gui {
    ...
    constructor(placehoder: String, fontrendererObj: UFontRenderer?, runnable: Runnable?) {
        ...
        this.placeholder = placehoder
        this.enabledColor = Konata.theme.textboxEnabled
        this.disabledColor = Konata.theme.textboxDisabled
    }

    constructor(placehoder: String) {
        this.font = Konata.fontManager.s18
        this.placeholder = placehoder
        this.enabledColor = Konata.theme.textboxEnabled
        this.disabledColor = Konata.theme.textboxDisabled
    }
    ...
    fun render(x: Float, y: Float, width: Float, height: Float, mouseX: Int, mouseY: Int) {
        ...
        if (this.visible) {
            if (Render2DUtils.isHovered(x, y, width, height, mouseX, mouseY)) {
                if (isFocused) {
                    btnColor.base(Konata.theme.textboxFocus)
                } else {
                    btnColor.base(Konata.theme.textboxHover)
                }
            } else {
