package cn.cutemic.konata.ui.notification
...
class Notification(val title: String, val desc: String, val type: Type, val time: Float) {
    ...
    init {
        anim.start(100.0, 0.0, 0.2f, cn.cutemic.konata.utils.math.animation.Type.EASE_IN_OUT_QUAD)
    ...
    fun draw(x: Float, y: Float) {
        ...
        if (anim.value == 0.0) {
            if (startTime == -1L) {
                ...
                anim.start(0.0, 100.0, 0.2f, cn.cutemic.konata.utils.math.animation.Type.EASE_IN_OUT_QUAD)
            }
        }
        ...
        Konata.fontManager.s18.drawStringWithShadow(
            title,
