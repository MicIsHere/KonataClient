package cn.cutemic.konata.utils.thirdparty.openai
...
fun requestClientAI(prompt: String, model: String, messages: JsonArray): Array<String?> {
    try {
        val sendPostRequest = HttpRequest.sendPostRequest(
            "${Konata.SERVICE_API}/chat?timestamp=${System.currentTimeMillis()}", Gson().toJson(messages), hashMapOf(
                "Content-Type" to "application/json",
                "username" to Konata.accountManager.username,
                "token" to Konata.accountManager.token,
                "model" to model,
                "prompt" to prompt
            )
        )
